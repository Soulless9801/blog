...
- `pages.ProblemsPage` contains fields `language` and `submission`, typical of program snippets.
- `pages.USACOProblemsPage` contains fields `language`, `submission`, `title`, `division`, and `link`, for USACO problem submission snippets.  
...
- `tags` denotes a dictionary mapping collection ID's to string value that can be stored and used to identify different document "types" within the same collection (documents saved with this specific editor will have this tag, so as to not be confused with other documents)
- `intended_collections` denotes a list of collections that are intended to be modified by this page
...
fields = {
    'title': {
        'type': 'lineedit', # check pages/page.py for presets
        'label': 'Title',
        'collection': ['posts', None] # this field will be modified in BOTH 'posts' colleciton and the current collection specified by 'collection'
    },
    'body': {
        'type': 'textedit',
        'label': 'Body',
        'on_change': self.render_markdown, # pass in custom functions
    },
    'language': {
        'type': 'dropdown',
        'label': 'Language',
        'options': ["English", "Spanish"], # options for a ComboBox
